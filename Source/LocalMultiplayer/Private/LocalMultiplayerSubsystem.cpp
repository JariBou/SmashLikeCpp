...
void ULocalMultiplayerSubsystem::CreateAndInitPlayers(ELocalMultiplayerInputMappingType MappingType)
{
	...
	for (int i = LocalMultiplayerSettings->KeyboardProfilesData.Num(); i < LocalMultiplayerSettings->KeyboardProfilesData.Num() + LocalMultiplayerSettings->nbMaxGamepads; ++i)
	{
		APlayerController* PlayerController = UGameplayStatics::CreatePlayer(GetWorld(), i);
		if (PlayerIndexFromKeyboardProfileIndex.FindKey(i))
		{
			// Reassign doode
